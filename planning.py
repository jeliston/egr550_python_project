...
class StateGatherer():
	...
	# We set up the names of each of the target frames that we want to pick or place 
        ...
        self.position = ["RedBox","BlueBox","GreenBox","DepositBoxRed","DepositBoxBlue","DepositBoxGreen"]
        self.place_target = ["DepositBoxRed","DepositBoxBlue","DepositBoxGreen"]
        self.pick_taget = ["RedBox","BlueBox","GreenBox"]
    ...
    def callback(self, data):
        ...
        # Map Gazebo and TF
        aux_idx = data.name.index(self.position[self.task_id])
    ...
    def sendGoal(self,req):
        ...
        if req.action == "place":
            if self.status == 1:
                try:
                    aux_pose = self.place_target[self.place_id]
                    self.place_id += 1
                    status = True
                    self.status = 0
                except:
            ...
            if self.status == 0:
                try:
                    aux_pose = self.pick_taget[self.task_id]
                    self.task_id += 1
                    status = True
                    self.status = 1
                except:
